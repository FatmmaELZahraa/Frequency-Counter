...
void demoqt::FindGlobal() {
    ...
    QString wordToFind = search->text(); // Get the word from the search bar

    // Convert the QString to std::string
    std::string wordToFindStdString = wordToFind.toStdString();

    // Check if the word exists in the global map
...
void demoqt::autocompletesearch()
{
    ...
    QTimer::singleShot(0, this, [this, matchingSuggestions]() {
        for (const QString& sug : matchingSuggestions) {
            suggestions->addItem(sug);
        }
        suggestions->showPopup();
        });
    QString selectedText = suggestions->currentText();
}
